...
func (hs *HTTPServer) GetHomeDashboard(c *models.ReqContext) Response {
	...
	if prefsQuery.Result.HomeDashboardId != 0 {
		...
		hs.log.Warn("Failed to get slug from database, %s", err.Error())
	}
...
func (hs *HTTPServer) CustomDeleteDashboard(c *models.ReqContext, cmd models.CustomDashboardCommand) Response {
	...
	hs.log.Info(fmt.Sprintf("Deleting the existing dashboard. Dashboard %s ", dash.Title))
	//guardian := guardian.New(dash.Id, c.OrgId, c.SignedInUser)
	//if canSave, err := guardian.CanSave(); err != nil || !canSave {
	//	return dashboardGuardianResponse(err)
	//}
	...
	hs.log.Info(fmt.Sprintf("Dashboard %s deleted", dash.Title))
	return JSON(200, util.DynMap{
		"title":   dash.Title,
		"message": fmt.Sprintf("Dashboard %s deleted", dash.Title),
	})
